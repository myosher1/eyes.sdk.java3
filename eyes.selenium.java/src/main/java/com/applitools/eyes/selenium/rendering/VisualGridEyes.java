...
public class VisualGridEyes implements IRenderingEyes, ISeleniumEyes {
    ...
    private Configuration configAtOpen;
    ...
    public WebDriver open(WebDriver webDriver) {
        ...
        ArgumentGuard.notNull(configProvider.getConfiguration().getTestName(), "testName");
        ArgumentGuard.notNull(configProvider.getConfiguration().getAppName(), "appName");

        initDriver(webDriver);

        configAtOpen = getConfigClone();
        ...
        if (configProvider.getConfiguration().getBatch() == null) {
            configProvider.getConfiguration().setBatch(new BatchInfo(null));
        }
        logger.verbose("getting all browsers info...");
        List<RenderBrowserInfo> browserInfoList = configProvider.getConfiguration().getBrowsersInfo();
        ...
        if (configProvider.getConfiguration().getBrowsersInfo() == null) {
            RectangleSize viewportSize = configProvider.getConfiguration().getViewportSize();
            configuration.addBrowser(new RenderBrowserInfo(viewportSize.getWidth(), viewportSize.getHeight(), BrowserType.CHROME, configProvider.getConfiguration().getBaselineEnvName()));
        }

        for (RenderBrowserInfo browserInfo : browserInfoList) {
            logger.verbose("creating test descriptor");
            RunningTest test = new RunningTest(createVGEyesConnector(browserInfo), browserInfo, logger, testListener);
    ...
    private Configuration getConfigClone() {
        return new Configuration(configProvider.getConfiguration());
    }

    private void ensureBrowsers() {
        if (this.configProvider.getConfiguration().getBrowsersInfo().isEmpty()) {
            this.configProvider.getConfiguration().getBrowsersInfo().add(new RenderBrowserInfo(viewportSize, BrowserType.CHROME));
        }
    }

    private void setViewportSize(EyesWebDriver webDriver) {
        viewportSize = configProvider.getConfiguration().getViewportSize();
    ...
    private IEyesConnector createVGEyesConnector(RenderBrowserInfo browserInfo) {
        ...
        EyesConnector VGEyesConnector = new EyesConnector(configProvider.getConfiguration(), this.properties, browserInfo);
        ...
        VGEyesConnector.setProxy(this.configProvider.getConfiguration().getProxy());
    ...
    public TestResults close(boolean throwException) {
        Collection<Future<TestResultContainer>> futures = close();
        return parseCloseFutures(futures, throwException);
    }

    public Collection<Future<TestResultContainer>> close() {
        Collection<Future<TestResultContainer>> futures;
        if (validateEyes()) {
            futures = closeAndReturnResults(false);
        } else {
...
//                    return firstCloseFuture.get().getTestResults();
    ...
    private Collection<Future<TestResultContainer>> closeAndReturnResults(boolean throwException) {
        ...
        logger.verbose("enter " + configProvider.getConfiguration().getBatch());
        try {
    ...
    public Collection<Future<TestResultContainer>> closeAsync() {
        ...
        Configuration configClone = getConfigClone();
        try {
            futureList = new ArrayList<>();
            for (RunningTest runningTest : testList) {
                logger.verbose("running test name: " + configProvider.getConfiguration().getTestName());
                ...
                FutureTask<TestResultContainer> closeFuture = runningTest.close(configClone);
    ...
    public void check(ICheckSettings checkSettings) {
        ...
        try {
            ...
            if ((b = checkSettingsInternal.isStitchContent()) != null) {
                isFullPage = b;
            } else if ((b = configProvider.getConfiguration().isForceFullPageScreenshot()) != null) {
                isFullPage = b;
            }
            ...
            for (RunningTest runningTest : filteredTests) {
                VisualGridTask checkVisualGridTask = runningTest.check(configClone, (ICheckSettings) checkSettingsInternal, regionsXPaths, source);
            ...
            this.renderingGridRunner.check((ICheckSettings) checkSettingsInternal, debugResourceWriter, scriptResult,
                    createVGEyesConnector(configProvider.getConfiguration().getBrowsersInfo().get(0)), visualGridTaskList, openVisualGridTasks, resultAsString,
    ...
    private void waitBeforeDomSnapshot() {
        int waitBeforeScreenshots = this.configProvider.getConfiguration().getWaitBeforeScreenshots();
        try {
    ...
    private ICheckSettingsInternal updateCheckSettings(ICheckSettings checkSettings) {
        ...
        if (matchLevel == null) {
            checkSettings = checkSettings.matchLevel(configProvider.getConfiguration().getMatchLevel());
        }

        if (fully == null) {
            checkSettings = checkSettings.fully((b = configProvider.getConfiguration().isForceFullPageScreenshot()) == null ? true : b);
        }

        if (sendDom == null) {
            checkSettings = checkSettings.sendDom((b = configProvider.getConfiguration().isSendDom()) == null ? true : b);
        }

        if (ignoreDisplacements == null) {
            checkSettings = checkSettings.ignoreDisplacements(configProvider.getConfiguration().getIgnoreDisplacements());
        }
    ...
    private synchronized List<VisualGridTask> addOpenTaskToAllRunningTest() {
        ...
        for (RunningTest runningTest : testList) {
            if (!runningTest.isOpenTaskIssued()) {
                VisualGridTask visualGridTask = runningTest.open(configAtOpen);
    ...
    public String getFullAgentId() {
        String agentId = configProvider.getConfiguration().getAgentId();
        if (agentId == null) {
